...
class Memory(object):  # stored as ( s, a, r, s_ ) in SumTree
    ...
    def batch_update(self, tree_idx, abs_errors):
        ...
        clipped_errors = np.minimum(abs_errors, self.abs_err_upper)  # 数值不大于1
        # TD error 转化成 priority
...
class DQNPrioritizedReplay:
    ...
    def store_transition(self, s, a, r, s_):
        ...
        self.occ.append(s_[1:5].mean())
        self.queue.append((s_[5:9]).mean())
        self.queue_var.append((s_[5:9]).var())
    ...
    def learn(self):
        ...
        if self.prioritized:
            ...
            self.memory.batch_update(tree_idx, abs_errors)     # update priority，abs_errors是新的优先级
        else:
    ...
    def plot_reward(self):
        ...
        plt.ylabel('Reward')
        plt.xlabel('transition numbers')
        plt.show()

    def plot_action(self):
        plt.plot(np.arange(len(self.action)), self.action)
        plt.ylabel('action')
        plt.xlabel('action numbers')
        plt.show()
    def return_results(self):
        return self.cost_his, self.rewards, self.action, self.occ, self.queue, self.queue_var
